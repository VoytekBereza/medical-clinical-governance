		...
		$.ajax({
			...
			success: function(result){
				...
				if(obj.error_msg){
					...
					if(obj.governance_hr != ''){ // If Governance founded
		...
		success: function( data ){
			...
			$.each( obj, function( key, value ) {
				...
				if(value.question != undefined){
					...
					// Place Question Statistics
					$( "#charts-for-pdf").append('<div> ' + value.statistics + '</div>');
					...
					// Draw chart to div [ charts_for_pdf ]
					chart.draw(data, {is3D: true, width: new_width, height: new_height, chartArea: {left:"22%", top:"1%", width:"100%", height:"100%"} });
