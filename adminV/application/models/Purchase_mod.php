...
class Purchase_mod extends CI_Model {
	...
	//Function add_pgd_to_temp(): Store the PGD into the temp database for renew purpsoe only
	public function add_pgd_to_temp($pgd_id = '',$user_id,$pgd_type){
		...
		if($pgd_type == 'ORAL'){
			...
			for($j=0;$j<count($get_oral_pgds_list);$j++){
				...
				$ins_data = array(
						...
						'package_type' => $this->db->escape_str(trim('O')),
				...
				$this->db->insert('user_order_tmp_details', $ins_data);
			...
			for($j=0;$j<count($get_premium_oral_pgds_list);$j++){
				...
				$ins_data = array(
						...
						'product_type' => $this->db->escape_str(trim('PGD')),
						'package_type' => $this->db->escape_str(trim('OP')),
						'is_package' => $this->db->escape_str(trim($is_package)),
						'created_date' => $this->db->escape_str(trim($created_date)), 
						'created_by_ip' => $this->db->escape_str(trim($created_by_ip))
				);
			...
			$ins_data = array(
					'user_id' => $this->db->escape_str(trim($user_id)),
					'product_id' => $this->db->escape_str(trim($pgd_id)),
					'product_type' => $this->db->escape_str(trim('PGD')),
					'package_type' => $this->db->escape_str(trim('V')),
					'is_package' => $this->db->escape_str(trim($is_package)),
					'created_date' => $this->db->escape_str(trim($created_date)), 
					'created_by_ip' => $this->db->escape_str(trim($created_by_ip))
			);
	...
	public function remove_pgd_from_temp($pgd_id,$user_id,$pgd_type){
		
		if($pgd_type == 'PGD'){
			
			$delete_product_from_temp = $this->delete_product_from_temp($user_id, $pgd_id, '');
		
		}elseif($pgd_type == 'ORAL'){
			
			$delete_product_from_temp = $this->delete_product_from_temp($user_id, '', 'O');
			
		}elseif($pgd_type == 'P_ORAL'){
			
			$delete_product_from_temp = $this->delete_product_from_temp($user_id, '', 'OP');
			
		}//end if($pgd_type == 'PGD')
		
		return true;
			
	}//end remove_pgd_from_temp($pgd_id,$user_id,$pgd_type)
	...
	public function delete_product_from_temp($user_id, $product_id = '', $pgd_type = ''){
		...
		if($pgd_type != '')
			$this->db->where('package_type', $pgd_type);
		
		$this->db->delete('user_order_tmp_details');
		
	}//end delete_product_from_temp($user_id, $pgd_id = '', $pgd_type = '')
