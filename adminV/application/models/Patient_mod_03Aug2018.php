...
class Patient_mod extends CI_Model {
   ...
   public function get_patient_grouped_history($patient_id,$order_status = ''){
	...
	public function decline_transaction($post){
		...
		$prescriber_name = ucwords(filter_string($get_default_prescriber['first_name']).' '.filter_string($get_default_prescriber['last_name']));
	...
	public function mark_transaction_complete($order_id){
		...
		$upd_arr = array(
						...
						'p_prescriber_reg_no' => $this->db->escape_str(trim($system_default_prescriber['registration_no'])),
						'p_prescriber_email' => $this->db->escape_str(trim($system_default_prescriber['email_address'])),
						'p_prescriber_contact_no' => $this->db->escape_str(trim($system_default_prescriber['mobile_no']))
					);
