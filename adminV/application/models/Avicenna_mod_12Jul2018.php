...
class Avicenna_mod extends CI_Model {
	...
	public function get_all_avicenna_ajax_call(){
		...
		*/
		...
		$select = array('first_name', 'last_name', 'mobile_no', 'email_address', 'registration_no', 'usertype.user_type', 'created_date', 'users.id', 'usertype.id as user_type_id');
			...
			foreach($select as $col)
            {
				...
				if($col == 'first_name'){
					...
					if($user_purchase_list['pgds']['package_purchased'] && $user_purchase_list['pgds']['package_purchased'] == 1){
						...
						$arr_data .= '<button class="btn btn-xs btn-success" onClick="assign_pgd_admin(\''.$user_id.'\', \'\', \'ORAL\', \'unassign\', \'Standard Oral PGD Package\',\''.$user_full_name.'\')" > O </button>';	
					} else {
					
						$arr_data .= '<button class="btn btn-xs btn-danger" onClick="assign_pgd_admin(\''.$user_id.'\', \'\', \'ORAL\', \'assign\', \'Standard Oral PGD Package\',\''.$user_full_name.'\')" > O </button>';	
					...
					if($user_purchase_list['pgds']['premium_package_purchased'] && $user_purchase_list['pgds']['premium_package_purchased'] == 1){
						...
						$arr_data .= '<button class="btn btn-xs btn-success" onClick="assign_pgd_admin(\''.$user_id.'\', \'\', \'P_ORAL\', \'unassign\', \'Premium Oral PGD Package\',\''.$user_full_name.'\')" > O+ </button>';	
						
					} else {
						$arr_data .= '<button class="btn btn-xs btn-danger" onClick="assign_pgd_admin(\''.$user_id.'\', \'\', \'P_ORAL\', \'assign\', \'Premium Oral PGD Package\',\''.$user_full_name.'\')" > O+ </button>';	

					}//end if($row[$key]['purchases']['pgds']['premium_package_purchased'] && $row[$key]['purchases']['pgds']['premium_package_purchased'] == 1)
					
					if($pT && $pT == 1){
						$arr_data .= '<button class="btn btn-xs btn-success" onClick="assign_pgd_admin(\''.$user_id.'\', \'10\', \'PGD\', \'unassign\', \'Travel Core 3.0\',\''.$user_full_name.'\')" > T </button>';
						
					} else {
						$arr_data .= '<button class="btn btn-xs btn-danger" onClick="assign_pgd_admin(\''.$user_id.'\', \'10\', \'PGD\', \'assign\', \'Travel Core 3.0\',\''.$user_full_name.'\')" > T </button>';	
					...
					if($F16 && $F16 == 1){
						...
						$arr_data .= '<button class="btn btn-xs btn-success" onClick="assign_pgd_admin(\''.$user_id.'\', \'15\', \'PGD\', \'unassign\', \'Seasonal 2017\',\''.$user_full_name.'\')" > F17 </button>';	
					 
					} else {
					 
						$arr_data .= '<button class="btn btn-xs btn-danger" onClick="assign_pgd_admin(\''.$user_id.'\', \'15\', \'PGD\', \'assign\', \'Seasonal 2017\',\''.$user_full_name.'\')" > F17 </button>';	
					 ...
					 if($F15 && $F15 == 1){
						 ...
						 $arr_data .= '<button class="btn btn-xs btn-success" onClick="assign_pgd_admin('.$row['id'].', "16", "PGD", "unassign","Seasonal PGD  2015",'.$user_full_name.')" > F17 </button>';
					...
					$row[] = '<a class="btn btn-info btn-xs pull-right"   href="'.SURL.'avicenna/add-edit-avicenna-user/'.$aRow['id'].'"><span class="glyphicon glyphicon-edit"></span></a>	';
				}
