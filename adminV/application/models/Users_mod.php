...
class Users_mod extends CI_Model {
  ...
  public function count_all_users($group_id = ''){
    
    $this->db->dbprefix('users');
    $this->db->select('count(id) as total');
    if($group_id != '') $this->db->where('buying_group_id',$group_id); 
    $this->db->from('users'); 
    $query = $this->db->get();
    ...
    */
    ...
    if(!empty($user_type) ){
        ...
        elseif($user_type == 'None Health Professional')
          $user_type_id = 7;
        elseif($user_type == 'Hong Kong Pharmacist')
          $user_type_id = 8;
    ...
    $data_db = $rResult->result_array();
      ...
      foreach($select as $col)
        ...
        //$user_purchase_list = $this->purchase->get_purchased_items_split_by_user(110);
        //print_this($user_purchase_list);        
        ...
        if($col == 'admin_verify_status'){
          ...
          if($aRow['user_type'] == '1'){
            ...
            $row[] = 'Pre-reg';
          }elseif($aRow['user_type'] == '8'){
            $row[] = 'Hong Kong Pharmacist';
          }else{
          ...
          // Check for HK : ID = 36 // Seasonal 2018
          $index_key = array_search(36, array_column($user_purchase_list['pgds']['pgd_list'], 'product_id'));
          if(strlen($index_key) > 0){

            $HK_data = $user_purchase_list['pgds']['pgd_list'][36];
            $HK_is_expired = ((strtotime(date('Y-m-d')) < strtotime($HK_data['expiry_date'])) || $HK_data['expiry_date'] == '0000-00-00') ? 0 : 1; //If PGD is expired marked as 1
            $HK = ($HK_is_expired) ? 0 : 1;
            
            $HK_is_expiring = verify_any_pgd_expiring(36, $HK_data['expiry_date']); //If it is about to expire
          ...
          // Check for HK : ID = 37 // 
          $index_key = array_search(37, array_column($user_purchase_list['pgds']['pgd_list'], 'product_id'));
          if(strlen($index_key) > 0){

            $SX_data = $user_purchase_list['pgds']['pgd_list'][37];
            $SX_is_expired = ((strtotime(date('Y-m-d')) < strtotime($SX_data['expiry_date'])) || $SX_data['expiry_date'] == '0000-00-00') ? 0 : 1; //If PGD is expired marked as 1
            $SX = ($SX_is_expired) ? 0 : 1;
            
            $SX_is_expiring = verify_any_pgd_expiring(37, $SX_data['expiry_date']); //If it is about to expire
          ...
          if($user_purchase_list['pgds']['package_purchased'] && $user_purchase_list['pgds']['package_purchased'] == 1 && ($user_purchase_list['pgds']['package_expired'] != '1') ){
            ...
            if($any_oral_pgd_expiring == 1){
              ...
              if(count($order_temp_details) > 0 ){
                $arr_data .= '<button title="PGD assigned in advance" class="btn btn-xs btn-warning"  type="button" onClick="renew_pgd_admin(\''.$user_id.'\', \'\', \'ORAL\', \'unassign\', \'Standard Oral PGD Package\',\''.$user_full_name.'\')" > O </button>';    
              }else{
                $arr_data .= '<button title="Assign PGD in advance" class="btn btn-xs btn-primary"  type="button" onClick="renew_pgd_admin(\''.$user_id.'\', \'\', \'ORAL\', \'assign\', \'Standard Oral PGD Package\',\''.$user_full_name.'\')" > O </button>';    
              }//end if($order_temp_details)
              
            }elseif($any_oral_pgd_expired == 1){
              
              $arr_data .= '<button title="Assign PGD" class="btn btn-xs btn-warning"  type="button" onClick="assign_pgd_admin(\''.$user_id.'\', \'\', \'ORAL\', \'assign\', \'Standard Oral PGD Package\',\''.$user_full_name.'\')" > O </button>';  
              
            }else{
              
              $arr_data .= '<button title="PGD has been assigned" class="btn btn-xs btn-success" type="button" onClick="assign_pgd_admin(\''.$user_id.'\', \'\', \'ORAL\', \'unassign\', \'Standard Oral PGD Package\',\''.$user_full_name.'\')" > O </button>';  
          ...
          if($user_purchase_list['pgds']['premium_package_purchased'] && $user_purchase_list['pgds']['premium_package_purchased'] == 1 && ($user_purchase_list['pgds']['premium_package_expired'] != '1')){
            ...
            if($any_prem_pgd_expiring == '1'){
              ...
              if(count($order_temp_details) > 0 ){
                ...
                $arr_data .= '<button title="PGD assigned in advance" class="btn btn-xs btn-warning"  type="button" onClick="renew_pgd_admin(\''.$user_id.'\', \'\', \'P_ORAL\', \'unassign\', \'Premium Oral PGD Package\',\''.$user_full_name.'\')" > O+ </button>';    
                
              }else{
                
                $arr_data .= '<button title="Assign PGD in advance" class="btn btn-xs btn-primary"  type="button" onClick="renew_pgd_admin(\''.$user_id.'\', \'\', \'P_ORAL\', \'assign\', \'Premium Oral PGD Package\',\''.$user_full_name.'\')" > O+ </button>';    
                
              }//end if($order_temp_details)
              
            }elseif($any_prem_pgd_expired == 1){
              
              $arr_data .= '<button title="Assign PGD" class="btn btn-xs btn-warning" type="button" onClick="assign_pgd_admin(\''.$user_id.'\', \'\', \'P_ORAL\', \'assign\', \'Premium Oral PGD Package\',\''.$user_full_name.'\')" > O+ </button>';               
              
            }else{
              
              $arr_data .= '<button title="PGD has been assigned" class="btn btn-xs btn-success" type="button" onClick="assign_pgd_admin(\''.$user_id.'\', \'\', \'P_ORAL\', \'unassign\', \'Premium Oral PGD Package\',\''.$user_full_name.'\')" > O+ </button>';                
          ...
          if($HK && $HK == 1){
            ...
            if($HK_is_expiring == '1'){
              ...
              $order_temp_details = get_order_temp_details($user_id, '36', '', '0');
              
              if(count($order_temp_details) > 0){
                
                $arr_data .= '<button class="btn btn-xs btn-warning" type="button" onClick="renew_pgd_admin(\''.$user_id.'\', \'36\', \'PGD\', \'unassign\', \'Hong Kong Flu PGD\',\''.$user_full_name.'\')" > HK </button>';
                
              }else{
                
                $arr_data .= '<button class="btn btn-xs btn-primary" type="button" onClick="renew_pgd_admin(\''.$user_id.'\', \'36\', \'PGD\', \'assign\', \'Hong Kong Flu PGD\',\''.$user_full_name.'\')" > HK </button>';  
                
              }//end if(count($order_temp_details) > 0)
              
            }else{
              
              $arr_data .= '<button class="btn btn-xs btn-success" type="button" onClick="assign_pgd_admin(\''.$user_id.'\', \'36\', \'PGD\', \'unassign\', \'Hong Kong Flu PGD\',\''.$user_full_name.'\')" > HK </button>';   
              
            }//end if($HK_is_expiring == '1') 
           
          } else {
           
            $arr_data .= '<button class="btn btn-xs btn-danger" type="button" onClick="assign_pgd_admin(\''.$user_id.'\', \'36\', \'PGD\', \'assign\', \'Hong Kong Flu PGD\',\''.$user_full_name.'\')" > HK </button>';  
          ...
          if($SX && $SX == 1){
            ...
            if($SX_is_expiring == '1'){
              ...
              $order_temp_details = get_order_temp_details($user_id, '37', '', '0');
              
              if(count($order_temp_details) > 0){
                
                $arr_data .= '<button class="btn btn-xs btn-warning" type="button" onClick="renew_pgd_admin(\''.$user_id.'\', \'37\', \'PGD\', \'unassign\', \'Liraglutide\',\''.$user_full_name.'\')" > SX </button>';
                
              }else{
                
                $arr_data .= '<button class="btn btn-xs btn-primary" type="button" onClick="renew_pgd_admin(\''.$user_id.'\', \'37\', \'PGD\', \'assign\', \'Liraglutide\',\''.$user_full_name.'\')" > SX </button>';  
                
              }//end if(count($order_temp_details) > 0)
              
            }else{
              
              $arr_data .= '<button class="btn btn-xs btn-success" type="button" onClick="assign_pgd_admin(\''.$user_id.'\', \'37\', \'PGD\', \'unassign\', \'Liraglutide\',\''.$user_full_name.'\')" > SX </button>';   
              
            }//end if($SX_is_expiring == '1') 
           
          } else {
           
            $arr_data .= '<button class="btn btn-xs btn-danger" type="button" onClick="assign_pgd_admin(\''.$user_id.'\', \'37\', \'PGD\', \'assign\', \'Liraglutide\',\''.$user_full_name.'\')" > SX </button>';  
          ...
          $arr_data .='<a href="'.SURL.'users/view-trainings/'.$get_user_data['user_type'].'/'.$aRow['id'].'" class="inline btn btn-xs btn-block btn-warning pull-right trainings-admin fancybox.ajax"> Trainings </a>'; 
      ...
      // [login_user_type] => avicenna
      $update_user_link = '';
      if($this->session->login_user_type == 'avicenna'){

        $update_user_link = '<a href="'.base_url().'avicenna/add-edit-avicenna-user/'.$aRow["id"].'" target="_blank" type="button" class="btn btn-info btn-xs pull-left"><span class="glyphicon glyphicon-edit"></span></a>';

      } else {
        
        $update_user_link = '<a href="'.base_url().'users/update-users/'.$aRow["id"].'" target="_blank" type="button" class="btn btn-info btn-xs pull-left"><span class="glyphicon glyphicon-edit"></span></a>';

      } // if($this->session->login_user_type == 'avicenna')

      $row[] = $update_user_link.'
  ...
  public function get_user_details_by_email($email_address){
    ...
    if(trim($email_address) !='')$this->db->where('users.email_address',$email_address);
  ...
  public function get_all_list_cqc_set($is_default = '', $is_active = ''){
    
    $this->db->dbprefix('cqc_sets');
    $this->db->select('cqc_sets.*, cqc_doctor.first_name AS doc_first_name, cqc_doctor.last_name AS doc_last_name, cqc_doctor.email_address AS doc_email_address, 
              cqc_pharmacist.first_name AS pharma_first_name, cqc_pharmacist.last_name AS pharma_last_name, cqc_pharmacist.email_address AS pharma_email_address'
            );
    ...
    if(trim($is_default) != '') $this->db->where('cqc_sets.is_default',$is_default);
    if(trim($is_active) != '') $this->db->where('cqc_sets.is_active',$is_active);
    ...
    if(trim($is_active) != '')
      ...
      return $get_cqc_list->result_array();
      
  }//end get_all_list_cqc_set($is_default = '')
  
  public function add_default_cqc_set($data){
    
    extract($data);
    
    $doctor_details = $this->get_user_details_by_email($doctor_email);
    $pharmacist_details = $this->get_user_details_by_email($pharmacist_email);
    
    $created_date = date('Y-m-d G:i:s');
    $created_by_ip = $this->input->ip_address();
    
    $folder_path = '../assets/images/cqc_stamps/';
    
    $stamp_image_name = '';
    
    if($_FILES['cqc_stamp']['name'] != ''){
      
      $allowed_exntension = array('jpg', 'jpeg', 'png', 'gif');

      $extension = pathinfo($_FILES['cqc_stamp']['name'], PATHINFO_EXTENSION);
      $extension = strtolower($extension);
      
      if(in_array($extension,$allowed_exntension)){
        
        $stamp_image_name = 'stamp_'.date('Ymdgis').'_'.rand().'.'.$extension;
        $destination_path = $folder_path.$stamp_image_name;
  
        @copy($_FILES['cqc_stamp']['tmp_name'], $destination_path);
        
      }//end if(in_array($extension,$allowed_exntension))
      
    }//end if($_FILES['cqc_stamp']['name'])

    $ins_data = array(
    
      'cqc_doctor_id' => $this->db->escape_str(trim($doctor_details['id'])),
      'cqc_pharmacist_id' => $this->db->escape_str(trim($pharmacist_details['id'])),
      'cqc_body' => $this->db->escape_str(trim($cqc_body)),
      'cqc_manager' => $this->db->escape_str(trim($cqc_manager)),
      'cqc_stamp' => $this->db->escape_str(trim($stamp_image_name)),
      'is_default' => $this->db->escape_str(trim('1')),
      'created_date' => $this->db->escape_str(trim($created_date)),
      'created_by_ip' => $this->db->escape_str(trim($created_by_ip)),
    );
    ...
    //Inserting Locum cities into the database. 
    $this->db->dbprefix('cqc_sets');
    $ins_into_db = $this->db->insert('cqc_sets', $ins_data);
  ...
  }//end add_default_cqc_set()
  
  public function mark_cqc_as_active($set_id){
    ...
    $this->db->dbprefix('cqc_sets');
    $this->db->where('is_default','1');
    $upd_into_db = $this->db->update('cqc_sets', $upd_data);
    
    $upd_data = array('is_active' => '1');  
  ...
  }//end mark_cqc_as_active($set_id)
  
  public function download_csv_file_users(){

    $this->load->dbutil();
    
    $qry_user_data = $this->db->query("SELECT kod_users.first_name AS FIRST_NAME,kod_users.last_name AS LAST_NAME, kod_users.email_address AS EMAIL, 
                      kod_users.registration_no AS REG_NO, kod_users.mobile_no AS CONTACT, 
                      kod_usertype.user_type AS USER_TYPE,
                      kod_buyinggroups.buying_groups AS BUYING_GROUP
                      FROM `kod_users` 
                      INNER JOIN `kod_usertype` ON `kod_users`.`user_type` = `kod_usertype`.`id` 
                      INNER JOIN `kod_buyinggroups` ON `kod_users`.`buying_group_id` = `kod_buyinggroups`.`id` 
    ...
    for($i=0; $i<count($count_all_record); $i++){
      ...
      $final_arr[$i]['email_address']= $count_all_record[$i]['email_address'];
      $final_arr[$i]['user_type']= $count_all_record[$i]['user_type'];
      $final_arr[$i]['registration_no']= $count_all_record[$i]['registration_no'];
      $final_arr[$i]['buying_groups']= $count_all_record[$i]['buying_groups'];
  ...
  public function get_order_temp_details($user_id, $product_id = '', $package_type = '', $is_package = ''){

    $this->db->dbprefix('user_order_tmp_details');
    $this->db->where('user_id',$user_id);
    ...
    if($is_package != '')
      $this->db->where('is_package',$is_package);
      
    $get_users= $this->db->get('user_order_tmp_details');
    //echo $this->db->last_query();     exit;
