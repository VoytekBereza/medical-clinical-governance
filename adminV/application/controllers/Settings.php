...
class Settings extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Online_prescriber_mod','online_prescriber');
	...
	public function online_doctor_prescribers(){
		
		//Active Buying Group List
		$get_prescriber_list = $this->online_prescriber->get_prescriber_list();
		$data['prescriber_list'] = $get_prescriber_list;
	
		// Add BreadCrumb Components
		$this->breadcrumbcomponent->add('Dashboard', base_url().'/dashboard');
		$this->breadcrumbcomponent->add('Online Prescriber List', '#');
		$data['breadcrum_data'] = $this->breadcrumbcomponent->output();
		...
		$this->stencil->paint('settings/online_doctor_prescribers',$data);
		
	}//end online_doctor_prescribers()
	
	public function add_edit_online_prescribers($pres_id = ''){
		
		if($pres_id != ''){
			
			$get_prescriber_list = $this->online_prescriber->get_prescriber_list($pres_id);
			$data['prescriber_detail'] = $get_prescriber_list;
			
		}//end if($group_id != '')
		
		// Add BreadCrumb Components
		$this->breadcrumbcomponent->add('Dashboard', base_url().'dashboard');
		$this->breadcrumbcomponent->add('Manage Online Doctor Prescribers ', base_url().'settings/online-doctor-prescribers');
		$this->breadcrumbcomponent->add('Add/ Edit Online Doctor Prescribers', base_url());
		$data['breadcrum_data'] = $this->breadcrumbcomponent->output();
		...
		$this->stencil->paint('settings/add_edit_online_prescribers',$data);
		
	}//end edit_online_prescribers()
	
	public function add_edit_online_prescriber_process(){
		...
		if(!$this->input->post()) redirect(base_url());
		 ...
		 if($pres_id == ''){
			 ...
			 $verify_if_email_exist = $this->online_prescriber->verify_if_prescriber_exist($email_address);
			...
			if($verify_if_email_exist){
				...
				redirect(SURL.'settings/add-edit-online-prescribers');
			
			}//end if($email_exist)
			
			$add_edit_online_doctor_prescriber = $this->online_prescriber->add_edit_online_doctor_prescriber($this->input->post());
			
			if($add_edit_online_doctor_prescriber){
				$this->session->set_flashdata('ok_message', 'New Online Prescriber added successfully.');	
			}else{
				$this->session->set_flashdata('err_message', 'Something went wrong, please try again later.');
			}//end if($add_edit_buying_group)
			
			redirect(SURL.'settings/online-doctor-prescribers');

		 }else{
			 
			 $verify_if_prescriber_exist = $this->online_prescriber->verify_if_prescriber_exist($email_address, $pres_id);
			 
			if($verify_if_prescriber_exist){
				
				$this->session->set_flashdata('err_message', 'Email ('.$email_address.') you have entered already exist. Use another one.');	
				redirect(SURL.'settings/add-edit-online-prescribers/'.$pres_id);
			...
			if($add_edit_online_doctor_prescriber){
				$this->session->set_flashdata('ok_message', 'Online Prescriber updated successfully.');	
			}else{
				$this->session->set_flashdata('err_message', 'Something went wrong, please try again later.');
			}//end if($add_edit_buying_group)
	...
	}//end add_edit_online_prescriber_process()
