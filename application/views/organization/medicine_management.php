...
<input type="text" readonly="readonly" onclick="$(this).select();" onfocus="$(this).select();" aria-describedby="sizing-addon1" value="<?php echo htmlentities('<script type="text/javascript"> var phd_embed_pharmacy_id = "'.$get_pharmacy_surgery_data['id'].'"; var phd_embed_pharmacy_name = "'.filter_string($get_pharmacy_surgery_data['pharmacy_surgery_name']).'"; var phd_embed_width = "100%"; var phd_embed_height = "780px"; var random_no = Math.floor((Math.random() * 100) + 1); var is_safari = navigator.userAgent.indexOf("Safari") > -1; var is_chrome = navigator.userAgent.indexOf("Chrome") > -1; if ((is_chrome) && (is_safari)) {is_safari = false;}   if (is_safari) { 	if (!document.cookie.match(/^(.*;)?\s*fixed\s*=\s*[^;]+(.*)?$/)) { 		document.cookie = "fixed=fixed; expires=Tue, 19 Jan 2038 03:14:07 UTC; path=/"; 		window.location.replace("'.VH_SURL.'embed/_safari_fix.php?rand="+random_no+""); 	} }</script><script type="text/javascript" src="'.VH_SURL.'embed/assets/js/embed.js"></script>'); ?>" class="form-control">
									</div>
                                    <br />
                                    <label>
	                                    <input type="checkbox" name="provide_delivery" id="provide_delivery" value="1" <?php echo ($get_pharmacy_surgery_data['online_delivery_status']) ? 'checked="checked"' : ''?>  /> Check if you provide delivery service
                                    </label>
...
<script>
	$('#provide_delivery').click(function(){
		
		provide_delivery = '0';
		if($('#provide_delivery').is(':checked'))
			provide_delivery = '1';
			
		$.ajax({
	
			type: "POST",
			url: SURL + "organization/update-provide-delivery",
			data: {'provide_delivery' : provide_delivery},
