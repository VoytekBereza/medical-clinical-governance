...
<a id="import_pharmacy_fancy_trigger" href="#fancy-pharmacy-import-content-div" style="display: none  ;" class=" btn btn-xxs btn-danger pharmacy_import_fancy"> Open Pharmacy Import </a>
<div style="display: none" class="row col-md-12">
    <div id="fancy-pharmacy-import-content-div">
        <h4 class="modal-title" id="elect_self_title">Import Pharmacies</h4>
        <hr />
		<span id="pharmacy_import_contents"></span>
    </div>
</div>
...
<div style="display: none" class="row col-md-12">
    <div id="fancy-content-div">
        <h4 class="modal-title" id="elect_self_title">Send Contract</h4>
        <hr />
        <form id="push_invitation_form" action="#" method="POST" >
            <div id="elect_self_textarea_container"><textarea id="governance_hr_textarea" name="governance_hr_text" class="textarea" placeholder="Enter Description" style="width: 800px; height: 400px"></textarea></div>
          <br />
            ...
            <div id="contract_notes_elect_self"></div>
            <div class="col-md-12 alert alert-info" id="contract_notes_others" style="font-size:15px">
	...
	if($this->session->is_owner){
		...
		if($organization_details_arr['superintendent_user_id']){
			...
			if(count($pharmacies_surgeries) == 0){
				...
				$ORGANIZATION_NO_PHARMACY_BLUE_TEXT = 'ORGANIZATION_NO_PHARMACY_BLUE_TEXT';
				$organization_no_pharmacy_blue_text = get_global_settings($ORGANIZATION_NO_PHARMACY_BLUE_TEXT); //Set from the Global Settings
				$organization_no_pharmacy_blue_text = filter_string($organization_no_pharmacy_blue_text['setting_value']);
				
				$organization_no_pharmacy_blue_text = str_replace('[ORGANIZATION_NAME]',ucwords(filter_string($this->session->organization['company_name'])),$organization_no_pharmacy_blue_text);

				$organization_no_pharmacy_blue_text = str_replace('[SI_NAME]',ucwords($organization_details_arr['superintendent_full_name']),$organization_no_pharmacy_blue_text);
				
				if(!$this->session->dismiss_message['org_no_pharmacy']){
?>
                    <p class="alert alert-info in alert-dismissable">
                        <a href="#" data-pharmacy="" data-org="<?php echo ($this->session->organization_id) ? $this->session->organization_id : $this->session->organization['id'] ?>" data-type="org_no_pharmacy" class="close dismiss_message" data-dismiss="alert" aria-label="close" title="close">×</a>
                        <i class="fa fa-info-circle"></i> <?php echo $organization_no_pharmacy_blue_text?>
                    </p>
<?php
				}//end if(!$this->session->dismiss_message['org_no_pharmacy'])
				
			}//end if(count($pharmacies_surgeries) == 0)
			
		}else{
			
			$ORGANIZATION_SI_BLUE_TEXT = 'ORGANIZATION_SI_BLUE_TEXT';
			$organization_si_blue_text = get_global_settings($ORGANIZATION_SI_BLUE_TEXT); //Set from the Global Settings
			$organization_si_blue_text = filter_string($organization_si_blue_text['setting_value']);
			
			$organization_si_blue_text = str_replace('[ORGANIZATION_NAME]',ucwords(filter_string($this->session->organization['company_name'])),$organization_si_blue_text);
			
			if(!$this->session->dismiss_message['org_no_si']){	
?>		
				<p class="alert alert-info in alert-dismissable">
                	<a href="#" data-pharmacy="" data-org="<?php echo ($this->session->organization_id) ? $this->session->organization_id : $this->session->organization['id'] ?>" data-type="org_no_si" class="close dismiss_message" data-dismiss="alert" aria-label="close" title="close">×</a>
	                <i class="fa fa-info-circle"></i> <?php echo $organization_si_blue_text?></strong></p>
<?php				
			
			}//end if(!$this->session->dismiss_message['org_no_si'])
...
?>
            <div class="row">
                <div class="col-md-12">
                    <h3 class="no_margin" style="margin:0"><?php echo ucwords(filter_string($this->session->organization['company_name']))?> 
                    <a href="<?php echo SURL?>organization/edit-organization" class="fancybox_view fancybox.ajax"><i class="fa fa-pencil"></i></a></h3><hr />
                </div>
            </div><!-- ./ row-->
            ...
            <div class="row">
                ...
                <?php 
                    if(!$check_si_contract_read_status['is_owner']){
                        //We do not show anything like governance for owners
                ?>
                    <div aria-label="..." role="group" class="btn-group">
                      <?php $check_si_stats = authenticate_user_panel($this->session->organization['superintendent_id'],$this->session->organization['id']);?>
                ...
                <?php 
                    }//end if(!$check_si_contract_read_status['is_owner'])
                ?>
                </div>
        ...
        <div id="invitation_response">
        	<div class="row" id="si_inv_container" <?php if($superintendent_invitation_arr['email_address_user'] == ''){ ?> style="display:none" <?php }?>>
            ...
            ?>
                    <div class="col-md-2">
                      <form name="elect_si_self_frm" id="elect_si_self_frm" method="post" action="<?php echo SURL?>organization/superintendent-elect-self">
                            <button type="button" class="btn btn-warning btn-sm btn-block elect-self-view-contract " data-target="<?php echo $this->session->is_owner ?>"  rel="P" name="SI" style="margin-top:18px;" > Elect Self </button>
                	...
                	<form name="superintendent_inv_frm" id="superintendent_inv_frm">
                        <div class="row">
                          <div class="col-sm-12 col-md-12 col-lg-12"> 
                            ...
                            <label>Send invitation to GPhC Superintendent or CQC Registered Manager</label>
                            <div class="input-group">
                                ...
                                <span class="input-group-btn" id="basic-addon3"><button class="btn btn-md btn-success" name="si_inv_btn" id="si_inv_btn" type="button">Invite Superintendent</button></span> 
                            </div>
                            <p><i>* Must be a Pharmacist or a Nurse</i></p>
                        ...
                        <div class="row <?php echo ($superintendent_invitation_arr) ? '' : 'hidden'?>" id="pending_invitation_container">
                            ...
                            <div class="col-sm-11 col-md-11 col-lg-11">
                                <div class="col-sm-8 col-md-8 col-lg-8 pull-left">
									<label class="text-danger" id="pending_inv_txt" >Invitation has been sent to <?php echo (filter_string($superintendent_invitation_arr['email_address_user']) != '' ) ?filter_string($superintendent_invitation_arr['email_address_user']) : filter_string($superintendent_invitation_arr['email_address']) ;?> waiting acceptance.</label>
                                </div>
...
?>
        <div class="row">
          <div class="col-sm-12 col-md-12 col-lg-12">
            ...
            <h3 style="margin:0"><?php echo ucwords(filter_string($this->session->organization['company_name']))?></h3>
            <hr />
            
          </div>
        </div><!-- ./ row-->
        
        <div class="row"><div class="col-md-12"><p><label class="text-success">Elect Superintendent/Responsible Person/Registered Manager: No one elected.</label></p></div></div><!-- ./ row-->
        <div class="row">
            <form name="elect_si_self_frm" id="elect_si_self_frm" method="post" action="<?php echo SURL?>organization/superintendent-elect-self">
                <div class="col-sm-2 col-md-2 col-lg-2">
                    <br />
                    <button type="button" class="btn btn-warning btn-sm btn-block elect-self-view-contract" rel="P" name="SI" data-target="<?php echo $this->session->is_owner ?>" > Elect Self</button>
                </div>
                <div class="col-sm-1 col-md-1 col-lg-1"> <h3>OR</h3></div>
                <input type="hidden" name="elect_self_hid" id="elect_self_hid" value="1" />
            </form>
                ...
                <div id="invitation_response">
                    <form name="superintendent_inv_frm" id="superintendent_inv_frm">
                        <div class="row <?php echo ($superintendent_invitation_arr) ? 'hidden' : ''?>" id="si_inv_container">
                          <div class="col-sm-12 col-md-12 col-lg-12"> 
                            <!-- Invite Superintendent input field -->
                            <label> Send invitation to Superintendent/Responsible Person/Registered Manager *</label>
                            <div class="input-group">
                              <input class="form-control" required="required" type="email" placeholder="Enter email address" id="email_address_si" name="email_address" />
                    ...
                    <div id="overlay" class="overlay hidden"><div class="col-md-12 text-center" style="margin-top:150px;"><span class="glyphicon glyphicon-refresh glyphicon-refresh-animate"></span></div></div>
                </div> 
            </div>
        </div><!-- ./ row-->
...
<div id="confirm_si_elect_self_modal" style="display:none">
  <h4 class="modal-title">Confirmation</h4>
  <p>Are you sure you want to elect yourself as a Superintendent of an organisation?</p>
  <div class="modal-footer">
...
?>
                <div class="row">
                    <div class="col-sm-12 col-md-12 col-lg-12"><p><h4><?php echo ucwords(filter_string($organization_details_arr['company_name']))?></h4><hr /></p></div>
                </div><!-- ./ row-->
<?php
...
<div class="row">
	...
	<?php 
        if($filter == 'All') { $filter_for = 'Pharmacies / Clinics'; } else if($filter == 'P'){ $filter_for = 'Pharmacies'; } else if($filter == 'S')
            $filter_for = 'Surgeries'; 
             
        if($filter!='All' && $filter!='Pharmacies / Clinics' && $filter!='P' && $filter!='S'){
    ?>
            <h4> Search result for: <?php echo $filter_for; ?></h4>
    <?php 
        } else {
    ?>
            <h3 class="no_margin" style="padding: 0;"> Locations (<?php echo $total_pharmacies_surgeries?>)</h3>
    <?php 
        } //end if($filter!='All' && $filter!='Pharmacies /Clinics' && $filter!='P' && $filter!='S')
    ?>
    </div>
	<div class="col-sm-2 col-md-2 col-lg-2"></div>
  
    <div class="col-sm-6 col-md-6 col-lg-6" style="margin-top: 5px;" >
    	<form class="form_validate" action="<?php echo base_url(); ?>organization/dashboard" method="get" id="list_pharmacies_surgeries_form">
            <div class="col-sm-4 col-md-4 col-lg-4">
                <select class="form-control" name="list_pharmacies_surgeries_filter" id="list_pharmacies_surgeries_filter">
                  ...
                  <option <?php echo ($filter == 'W') ? 'selected="selected"' : '' ?> value="W">Wholesale Dealer Authorisation</option>
                  <!--<option <?php echo ($filter == 'U') ? 'selected="selected"' : '' ?> value="U">User</option> -->
                </select>
            </div>
            <div class="col-sm-8 col-md-8 col-lg-8">
                <div class="input-group">
                  <input id="pharmacy_surgery_search" name="pharmacy_surgery_search" type="text" placeholder="Search" value="<?php echo $this->input->get('pharmacy_surgery_search')?>" class="form-control" />
                  <span class="input-group-btn">
                  <button class="btn btn-default" type="submit" id="search_btn" name="search_btn">Go</button>
                  <input type="hidden" name="is_src" value="1" readonly="readonly" />
                  </span> </div>
            </div>
      </form>
    </div>
...
if(!empty($pharmacies_surgeries)){
	...
	if(strlen($no_manager_key) > 0){
		...
		$ORGANIZATION_ADD_MANAGER_BLUE_TEXT = 'ORGANIZATION_ADD_MANAGER_BLUE_TEXT';
		$organization_add_manager_blue_text = get_global_settings($ORGANIZATION_ADD_MANAGER_BLUE_TEXT); //Set from the Global Settings
		$organization_add_manager_blue_text = filter_string($organization_add_manager_blue_text['setting_value']);
		
		//print_this($this->session);
		if(!$this->session->dismiss_message['org_no_manager']){
?>
            <p class="alert alert-info in alert-dismissable">
            	<a href="#" data-pharmacy="<?php echo $pharmacy_surgery['id']; ?>" data-org="<?php echo ($this->session->organization_id) ? $this->session->organization_id : $this->session->organization['id'] ?>" data-type="org_no_manager" class="close dismiss_message" data-dismiss="alert" aria-label="close" title="close">×</a>
                <i class="fa fa-info-circle"></i> <?php echo $organization_add_manager_blue_text?>
            </p>
...
?>
        ...
        <div class="row" >
            <div class="col-sm-12 col-md-12 col-lg-12" >
	            <h4 class="text-default" style="margin:0" > 
					<?php echo ($pharmacy_surgery['pharmacy_surgery_name'] != '') ? ucwords(filter_string($pharmacy_surgery['pharmacy_surgery_name'])) : '' ?>, <?php echo ($pharmacy_surgery['postcode'] != '') ? filter_string($pharmacy_surgery['postcode']) : '' ?> 
                   ...
                   <?php if(count($pharmacies_surgeries) > 5){?>
                       <span class="pharmacy_setting_link pull-right"><a class="expand-btn" href="javascript:;" onclick="$('#pharmacy_container_<?php echo filter_string($pharmacy_surgery['id']) ?>').toggle('medium');" > +Expand </a></span>
                   <?php }//end if(count($pharmacies_surgeries) > 5)?>
            	</h4>
            </div>
        </div><!-- ./ row-->
          <!-- Modal -->
        <div id="myModal<?php echo $pharmacy_surgery['id']; ?>" style="display:none">
            <h4 class="modal-title">Confirmation</h4>
            <p>Are you sure you want to delete the selected <?php echo ($pharmacy_surgery['type'] == 'P') ? 'Pharmacy' : 'Clinic' ?> <strong><?php echo ucwords(filter_string($pharmacy_surgery['pharmacy_surgery_name']))?></strong>?</p>
            ...
            <div <?php echo (count($pharmacies_surgeries) > 5) ? 'style="display:none"' : '' ?> id="pharmacy_container_<?php echo filter_string($pharmacy_surgery['id'])?>">
            	<div class="row"><div class="col-sm-12 col-md-12 col-lg-12"><h4 class="text-default"> Staff </h4></div></div>
			<?php 
                if($pharmacy_surgery['manager_id'] == 0){ 
                
                    if(empty($pharmacy_surgery['manager_invitations'])) {
                ?>
                        <strong class="text-success hidden" id="self_manager_row_<?php echo $pharmacy_surgery['id']; ?>"><?php echo ($pharmacy_surgery['type'] == 'P') ? 'Pharmacy' : 'Clinic' ?> <span class="hidden" id="manager_info_span_<?php echo $pharmacy_surgery['id']; ?>"></span></strong>
                        ...
                        <div class="col-sm-10 col-md-10 col-lg-10" id="invitation_response_m_<?php echo $pharmacy_surgery['id']; ?>" >
                            <div class="row">
                                <div class="col-md-2 text-left" style="padding-left:0px">
                                    <button type="button" class="btn btn-sm btn-block btn-warning elect-self-view-contract" data-target="<?php echo $this->session->is_owner?>" rel="<?php echo $pharmacy_surgery['type']; ?>" name="M" value="<?php echo $pharmacy_surgery['id']; ?>" > Elect Self </button>
                                </div>
                        ...
                        <div class="row hidden" id="pending_invitation_container_m_<?php echo $pharmacy_surgery['id']; ?>">
                            <div class="col-sm-12 col-md-12 col-lg-12">
                                <div class="col-sm-8 col-md-8 col-lg-8 pull-left">
                                  <label class="text-danger" id="pending_inv_txt" >Invitation has been sent to <span id="invitation_sent_to_span_<?php echo $pharmacy_surgery['id']; ?>"><?php echo ($pharmacy_surgery['manager_invitations'][0]['user_email_address'] != '') ? $pharmacy_surgery['manager_invitations'][0]['user_email_address'] : '' ?></span> waiting acceptance.</label>
                                </div>
            ...
            <?php 
                ...
                ?>
                        ...
                        <div class="col-sm-10 col-md-10 col-lg-10 hidden" id="invitation_response_m_<?php echo $pharmacy_surgery['id']; ?>" >
                            <div class="row">
                                <div class="col-md-2 text-left" style="padding-left:0px">
                                    <button type="button" class="btn btn-warning elect-self-view-contract" data-target="<?php echo $this->session->is_owner?>" rel="<?php echo $pharmacy_surgery['type']; ?>" name="M" value="<?php echo $pharmacy_surgery['id']; ?>" > Elect Self </button>
                                </div>
                                <div class="col-sm-1 col-md-1 col-lg-1" style="padding-top:2px; font-size:18px">OR</div>
                                <div class="col-md-8">
                                    <div class="input-group">
                    ...
                    <div class="row">
                        ...
                        <div class="btn-group" role="group" aria-label="..."> 
                          ...
                          <?php 
                            ...
                            if(!$pharmacy_surgery['manager_is_owner']){
                                //If not owner show governance section
                          ?>
                              <!-- Manager Governance read status -->
                          ...
                          <?php 
                            }//end if(!$pharmacy_surgery['manager_is_owner'])
                          ?>
                            ...
                            if($pharmacy_surgery['manager_is_prescriber'] == 0 && in_array($pharmacy_surgery['manager_usertype_id'], $non_presriber_usertype_arr)){ // if the manager is not a prescriber ?>
                              <div class="btn-group tooltip-default" role="group" data-toggle="tooltip" data-placement="top" title="Here you can assign individual PGDs to staff.">
                                ...
                                <ul class="dropdown-menu mydropdown-menu">
                                  ...
                                  <li> <a href="<?php echo SURL?>organization/single-product-checkout/OP/1/<?php echo $pharmacy_surgery['manager_id'] ?>/<?php echo $pharmacy_surgery['id'] ?>" class="btn btn-xxs btn-danger">Standard Oral PGD Package <span class="glyphicon glyphicon-shopping-cart"></span></a> </li>
                                  <?php
                                ...
                                <!-- Preimum Oral PGD Package -->
                                  <?php	
                                    if($pharmacy_surgery['purchased_items_split_arr']['pgds']['premium_package_purchased'] == 1 && $pharmacy_surgery['premium_purchased_items_split_arr']['pgds']['premium_package_expired'] == 0){
                            
                                            if(count($premium_oral_pgd_arr) > 0){ 
                                  
                                                for($i=0;$i<count($premium_oral_pgd_arr);$i++){
                                            
                                                    $expiry_date = $pharmacy_surgery['purchased_items_split_arr']['pgds']['pgd_list'][$premium_oral_pgd_arr[$i]['id']]['expiry_date'];
                                                    $is_purchased = ($pharmacy_surgery['purchased_items_split_arr']['pgds']['pgd_list'][$premium_oral_pgd_arr[$i]['id']]) ? 1 : 0; //If this PDG is puchased mark as 1.
                                                    $is_expired = (strtotime(date('Y-m-d')) <= strtotime($expiry_date)) ? 0 : 1; //If PGD is expired marked as 1
                                                    $is_quiz_passed = $pharmacy_surgery['purchased_items_split_arr']['pgds']['pgd_list'][$premium_oral_pgd_arr[$i]['id']]['is_quiz_passed']; //If quiz assesment is passed marked as 1
                                                    $doctor_approval = $pharmacy_surgery['purchased_items_split_arr']['pgds']['pgd_list'][$premium_oral_pgd_arr[$i]['id']]['doctor_approval']; //Have doc approved the PGD after assesment
                                                    $pharmacist_approval = $pharmacy_surgery['purchased_items_split_arr']['pgds']['pgd_list'][$premium_oral_pgd_arr[$i]['id']]['pharmacist_approval']; //Have pharmacist approved the PGD after assesment
                                                            
                                                    if($is_quiz_passed && $doctor_approval && $pharmacist_approval){ // If Purchased and not expired and assesment test is passed, doc and pharmacist have approved the PGD assesment
                                                                    
                                    ?>
                                        <li> <a href="javascript:;" class="btn btn-xxs btn-success"><?php echo filter_string($premium_oral_pgd_arr[$i]['pgd_name'])?></a> </li>
                                  <?php
									...
									// If Purchased and not expired and assesment test is not passed
									?>
                                  			<li> <a href="javascript:;" class="btn btn-xxs btn-warning"><?php echo filter_string($premium_oral_pgd_arr[$i]['pgd_name'])?> </a> </li>
                                  <?php
                                            } //end if($is_quiz_passed)
                                  ...
                                  <li> <a href="<?php echo SURL?>organization/single-product-checkout/POP/1/<?php echo $pharmacy_surgery['manager_id'] ?>/<?php echo $pharmacy_surgery['id'] ?>" class="btn btn-xxs btn-danger">Premium Oral PGD Package <span class="glyphicon glyphicon-shopping-cart"></span></a> </li>
                                  <?php
                                                                
                                    } // End - if($pharmacy_surgery['premium_purchased_items_split_arr']['pgds']['premium_package_purchased'] == 1 && $pharmacy_surgery['premium_purchased_items_split_arr']['pgds']['premium_package_expired'] == 0)
                                ?>
                              ...
                              <?php	
                                ...
                                if(count($training_courses_arr) > 0){
                                    ...
                                    <?php
                                                ...
                                                if(filter_string($pharmacy_surgery['purchased_items_split_arr']['training'][$training_courses_arr[$i]['id']]['is_quiz_passed']) == 1){
                                    ?>
                        ...
                        <!-- Start - Manager [ PGDs & Trainings ]  --> 
                        <?php $count_no_of_staff = count($pharmacy_surgery['pharmacy_surgery_staff']);?>
                        ...
                        <div id="expand_pharmacy_<?php echo $pharmacy_surgery['id']; ?>" style=" <?php echo ($count_no_of_staff > 5) ? 'display: none;' : ''?> " > 
                          ...
                          <div id="view_staff_<?php echo $pharmacy_surgery['id']; ?>" style="display:block;"> 
                            ...
                            <div class="btn-group" role="group" aria-label="...">
                                <?php 
                                    if(!$member['staff_member_is_owner']){
                                        
                                        //If staff member is an owner do not show teh governance
                                ?>
                                ...
                                <?php		
                                    }//end if(!$member['staff_member_is_owner'])
                                ?>
                              ...
                              <?php 
                                ...
                                if($member['staff_member_is_prescriber'] == 0 && in_array($member['staff_member_user_type_id'], $non_presriber_usertype_arr)){ ?>
                                  <div class="btn-group tooltip-default" role="group" data-toggle="tooltip" data-placement="top" title="Here you can assign individual PGDs to staff.">
                                   ...
                                   ?>
                                      <li> <a href="<?php echo SURL?>organization/single-product-checkout/OP/1/<?php echo filter_string($member['staff_member_id']) ?>/<?php echo $pharmacy_surgery['id'] ?>" class="btn btn-xxs btn-danger">Standard Oral PGD Package <span class="glyphicon glyphicon-shopping-cart"></span></a> </li>
                                      <?php
                                                                                    
                                    } // End - if($member['purchased_items_split_arr']['pgds']['package_purchased'] == 1 && $member['purchased_items_split_arr']['pgds']['package_expired'] == 0)
                                        ?>
                                        
                                        <!-- Premium Oral PGD Package -->

                                      <?php 
                                        if($member['purchased_items_split_arr']['pgds']['premium_package_purchased'] == 1 && $member['purchased_items_split_arr']['pgds']['premium_package_expired'] == 0){
                                        
                                            if(count($premium_oral_pgd_arr) > 0){
                                                
                                                for($i=0;$i<count($premium_oral_pgd_arr);$i++){
                                                
                                                    $expiry_date = $member['purchased_items_split_arr']['pgds']['pgd_list'][$premium_oral_pgd_arr[$i]['id']]['expiry_date'];
                                                    $is_purchased = ($member['purchased_items_split_arr']['pgds']['pgd_list'][$premium_oral_pgd_arr[$i]['id']]) ? 1 : 0; //If this PDG is puchased mark as 1.
                                                    $is_expired = (strtotime(date('Y-m-d')) <= strtotime($expiry_date)) ? 0 : 1; //If PGD is expired marked as 1
                                                    $is_quiz_passed = $member['purchased_items_split_arr']['pgds']['pgd_list'][$premium_oral_pgd_arr[$i]['id']]['is_quiz_passed']; //If quiz assesment is passed marked as 1
                                                    $doctor_approval = $member['purchased_items_split_arr']['pgds']['pgd_list'][$premium_oral_pgd_arr[$i]['id']]['doctor_approval']; //Have doc approved the PGD after assesment
                                                    $pharmacist_approval = $member['purchased_items_split_arr']['pgds']['pgd_list'][$premium_oral_pgd_arr[$i]['id']]['pharmacist_approval']; //Have pharmacist approved the PGD after assesment
                                                            
                                                    if($is_quiz_passed && $doctor_approval && $pharmacist_approval){ // If Purchased and not expired and assesment test is passed, doc and pharmacist have approved the PGD assesment
                                                        
                                            ?>
		                                        <li> <a href="javascript:;" class="btn btn-xxs btn-success"><?php echo filter_string($premium_oral_pgd_arr[$i]['pgd_name'])?></span></a> </li>
                                        <?php
                                               } else {
                                                        // If Purchased and not expired and assesment test is not passed
										?>
                                        		<li> <a href="javascript:;" class="btn btn-xxs btn-warning"><?php echo filter_string($premium_oral_pgd_arr[$i]['pgd_name'])?></span></a> </li>
                                        <?php
                                                } //end if($is_quiz_passed)
                                   ...
                                   ?>
                                      <li> <a href="<?php echo SURL?>organization/single-product-checkout/POP/1/<?php echo filter_string($member['staff_member_id']) ?>/<?php echo $pharmacy_surgery['id'] ?>" class="btn btn-xxs btn-danger">Premium Oral PGD Package <span class="glyphicon glyphicon-shopping-cart"></span></a> </li>
                                      <?php
                                                                                    
                                    } // End - if($member['purchased_items_split_arr']['pgds']['premium_package_purchased'] == 1 && $member['purchased_items_split_arr']['pgds']['premium_package_expired'] == 0)
                                        ?>
                        ...
                        <?php
                                  ...
                                  <?php	
                                        ...
                                        if(count($training_courses_arr) > 0){
                                            ...
                                            <?php
                                                        ...
                                                        if(filter_string($member['purchased_items_split_arr']['training'][$training_courses_arr[$i]['id']]['is_quiz_passed']) == '1'){
                                                    ?>
                                                            <li> <a href="javascript:;" class="btn btn-xxs btn-success"> <?php echo filter_string($training_courses_arr[$i]['course_name'])?></a> </li> 
                                            <?php
                                                        } else {
                            ...
                            ?>
                                		<p class="alert alert-danger">No Staff Members are in this location</p>
                            <?php
                            ...
                            <!-- Start - List Pending Invitation of Staff --> 
                                        <span id="staff_pending_invitation_span_<?php echo $pending_invitation['id']; ?>" class="pull-left" style="margin-top: 10px"> 
                                        ...
                                        <label class="text-danger "> 
                                        	<?php echo ($pending_invitation['request_changes'] == '1') ? 'A request for contract change has been received by ' : 'Awaiting Contract Agreement'?> - <?php echo filter_string($pending_invitation['user_email_address'])?>
                                        </label>
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                	...
                                	<?php 
										$ORGANIZATION_ADD_NEW_STAFF_BLUE_TEXT = 'ORGANIZATION_ADD_NEW_STAFF_BLUE_TEXT';
										$organization_add_new_staff_blue_text = get_global_settings($ORGANIZATION_ADD_NEW_STAFF_BLUE_TEXT); //Set from the Global Settings
										$organization_add_new_staff_blue_text = filter_string($organization_add_new_staff_blue_text['setting_value']);
									
										if(!$this->session->dismiss_message[$pharmacy_surgery['id']]['org_add_staff']){
									?>
                                	
                                        <p class="alert alert-info in alert-dismissable">
                                            <a href="#" class="close dismiss_message" data-pharmacy="<?php echo $pharmacy_surgery['id']; ?>" data-org="<?php echo ($this->session->organization_id) ? $this->session->organization_id : $this->session->organization['id'] ?>" data-type="org_add_staff" data-dismiss="alert" aria-label="close" title="close">×</a>
                                            <i class="fa fa-info-circle"></i> <?php echo $organization_add_new_staff_blue_text?>
                                        </p>
                                        
                                    <?php 
										}//end if(!$this->session->dismiss_message['org_add_staff'])
									?>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <!--<div class="col-sm-2 col-md-2 col-lg-2" style="padding:0">
                                        <button type="button" class="btn btn-warning elect-self-view-contract btn-block" data-target="< ?php echo $this->session->is_owner?>" rel="< ?php echo $pharmacy_surgery['type']; ?>" name="ST" value="< ?php echo $pharmacy_surgery['id']; ?>" > Elect Self </button>
                                    </div>
            ...
            <?php 
				//If any request change is received within the pharmacy
				if($request_change_flag && $request_change_flag == 1){
			?>
            <div class="text-danger" style="padding:5px; margin-top:10px;"><strong><i class="fa fa-exclamation-triangle"></i> A request for contract change has been received</strong></div>
            
            <?php }//end if($request_change_flag && $request_change_flag == 1)?>
		</div>
...
?>
        <div class="row">
            <div class="col-md-12 text-right" style="margin-bottom: 10px" >
                <div style="background-color:#F5F5F5; width:100%" class="pull-left"><?php echo $pagination_links?></div>
            </div>
         </div>
<?php	
 } else { 
...
<?php 
	} // else - if(!empty($pharmacies_surgeries)) 
?>
</div>
...
<div class="row">
    ...
    <div class="col-md-1 col-lg-1"> 
     ...
     <div class="col-md-4 col-lg-4"> 
        ...
        <a href="<?php echo base_url(); ?>organization/edit-pharmacy-surgery" class="btn btn-sm btn-primary fancybox_view fancybox.ajax "><i class="fa fa-plus"></i> Add Location </a>
        
        <a href="javascript:;" onclick="$('#import_file').click()" class="btn btn-sm btn-warning pull-right"><i class="fa fa-plus"></i> Import Pharmacies</a> 
        <form name="import_pharmacy_frm" id="import_pharmacy_frm" method="post">
	        <input type="file" id="import_file" name="import_file" class="hidden" onchange="process_import();" />
        </form>
        <a href="<?php echo CSV ?>import-sample-csv.csv" class="pull-right"><small>Sample CSV format</small></a>
        <p class="pull-right hidden" id="import_response"></p>
        
        <script>
			function process_import(){

				var form = $('#import_pharmacy_frm')[0]; // You need to use standart javascript object here
				...
				$.ajax({
					...
					processData: false,
					cache: false,
					url: SURL + "organization/import-pharmacy-list",
					data: form_data,
				
					beforeSend : function(result){
						$("#import_response").removeClass("hidden");
						$("#import_response").html('<small class="text-danger">Processing, please wait!</small>');
					...
					success: function(response){
						...
						if(obj.error == 1){
							...
							$("#import_response").removeClass("hidden");
							$("#import_response").html('<small class="text-danger">'+obj.error_message+'</small>');
							
						}else if(obj.error == 0){

							$("#import_response").addClass("hidden");
							$("#import_response").html('');
							
							$("#pharmacy_import_contents").html(obj.pharmacy_list);
							$("#import_pharmacy_fancy_trigger").trigger('click');
...
?>
            <div class="row">
            	<div class="col-sm-12 col-md-12 col-lg-12"><p><h3><?php echo ucwords(filter_string($organization_details_arr['company_name']))?></h3><hr /></p></div>
            </div><!-- ./ row--> 
<?php 
		}//end if(!$this->session->owner)
?>
  ...
  <div class="col-sm-12 col-md-12 col-lg-12"> 
    ...
    <!-- Start - Governance tabs body -->
        <div id="sops">
            ...
            <p>
	            <style>.tree li a{ color:#FFF }.tree li{ font-weight:normal; color:#5CB85C; }</style>
        ...
        ?>
                                    <li id="li_cat_<?php echo $split_category_name[0]?>"><a href="#" id="cat_<?php echo $split_category_name[0]?>" style="font-weight:bold;color:#5CB85C"><?php echo filter_string($split_category_name[1]);?></a>
                                        <ul>
                            <?php		
										//print_this($user_sop_read_list); exit;
                            ...
                            <?php 					
                                            ...
                                            if(is_numeric($index_key) && $user_sop_read_list[$index_key]['is_signed']){
