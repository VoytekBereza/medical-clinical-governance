                    ...
                    <div class="panel panel-default"> 
                      ...
                      <div class="panel-heading" ><a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion" href="#collapseOne"><strong>Standard Oral PGD Package</strong> </a> <a href="<?php echo SURL?>pgd/package-info/1" class="btn btn-xs fancybox_pgd_detail fancybox.ajax"> <span class="glyphicon glyphicon-info-sign"></span></a>
                         <?php 
                            ...
                            $any_oral_pgd_expired = is_any_oral_pgd_package_expired($oral_pgd_arr, $purchased_items_split_arr);
                      ...
                      <div class="panel-heading">
                        <a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion" href="#collapseTwo"><strong>Premium Oral PGD Package</strong></a> <a href="<?php echo SURL?>pgd/package-info/2" class="btn btn-xs fancybox_pgd_detail fancybox.ajax"> <span class="glyphicon glyphicon-info-sign"></span></a>
                         <?php 
                            ...
                            $any_prem_pgd_expired = is_any_prem_pgd_package_expired($premium_oral_pgd_arr, $purchased_items_split_arr);
...
<?php
// Verify if the user is loggedin => If yes then show this popup for new terms if not already agreed
if($this->session->id){

    // Verify if user already has agreed or not
    $user_details = get_user_details_new($this->session->id);

    ?>

    <!-- Agree to new terms -->
    <a href="#new_terms_pop" id="open_new_terms_pop" class="fancy_inline_custom hidden">Open</a>

    <div id="new_terms_pop" style="display:none;">
        
        <div class="col-md-12">
            <?php
            $cms_data_arr = $this->cms->get_cms_page('terms--conditions');
            // echo '<pre>'; print_r($cms_data_arr);
            echo '<h1>'.filter_string($cms_data_arr['cms_page_arr']['page_title']).'</h1>';
            ?>
        </div>

        <div class="col-md-12">

            <div style="max-height: 300px; overflow-y: scroll; background-color: #f5f5f5 !important; padding: 15px;">

                <?php 
                echo filter_string($cms_data_arr['cms_page_arr']['page_description']);
                ?>
        ...
        <div class="col-md-12">
            ...
            <p>
                <?php
                $NEW_TERMS_AGREE_POPUP_TEXT = 'NEW_TERMS_AGREE_POPUP_TEXT';
                $new_terms = get_global_settings($NEW_TERMS_AGREE_POPUP_TEXT); //Set from the Global Settings
                $new_terms_txt = filter_string($new_terms['setting_value']);
                echo $new_terms_txt;
                ?>
            </p>

            <a href="<?php echo SURL; ?>home/new_terms_process/<?php echo $this->session->id; ?>" class="btn btn-primary btn-lg btn-block">Accept and Proceed</a>
    ...
    <script>
        ...
        $(window).load(function(){
            ...
            <?php 
                if(!$user_details['new_terms_agree']){
            ?>
                    $('#open_new_terms_pop').click();
            <?php       
                }//end if(!$user_details['new_terms_agree'])
            ?>
            
        });

        $(document).ready(function(){
            $('.fancy_inline_custom').fancybox({
                minWidth  : '70%',
                maxWidth  : '70%',
                closeBtn :  false,
