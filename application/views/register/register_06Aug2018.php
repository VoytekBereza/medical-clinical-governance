    ...
    <form data-toggle="validator" role="form" method="post" name="hcp_org_frm" id="hcp_org_frm"  action="<?php echo SURL?>register/register-process" autocomplete="off">
      ...
      <div class="form-group has-feedback <?php if($session_data['registration_no']==""){?> hidden  <?php }?>"  id="gmc_no_div">
        <label for="gmc-gphc-nmc">
          <?php if($session_data['user_type']=='1'){ echo 'GMC No*';} else if($session_data['user_type']=='2' || $session_data['user_type']=='6') { echo 'GPhC No*';} else if($session_data['user_type']=='3') { echo 'NMC No*';} ?>
        </label>
      ...
      <div class="form-group has-feedback">
        ...
        <input type="digits" placeholder="Mobile Number" name="mobile_no" id="mobile_no"  required="required" value="<?php echo $session_data['mobile_no'];?>" data-error="Please use allowed characters (Numbers, should start with 07 and length should be 11 numbers)" class="form-control"  pattern="^07(?=.*[0-9])[0-9]{9,}$" maxlength="11">
        <span class="glyphicon form-control-feedback" aria-hidden="true"></span>
        <div class="help-block with-errors"></div>
      </div>
      <div class="form-group has-feedback">
