...
class Registers extends MY_Organization_Controller {
	...
	public function index($drug_id = '', $page_no = ''){
		...
		*/
		...
		$this->stencil->js('kod_scripts/registers');
	...
	public function register_load_more(){

		extract($this->input->post());

		$current_page = ($current_page) ? $current_page : 1 ;
		 
		$list_all_entry = $this->registers_entry->list_register_all_entery($this->session->id,$this->session->pharmacy_surgery_id,$drug_id_select_box, '','',$keyword);
		$data['total_listing_count'] = count($list_all_entry);
		
		//$no_of_rec_per_page = JOBS_ENTERIES_PER_PAGE;
		$no_of_rec_per_page = 10;
		$data['no_of_rec_per_page'] = $no_of_rec_per_page;

		//Pagination Settings
		$total_num_of_pages = ceil(count($list_all_entry) / $no_of_rec_per_page );
		$data['total_num_of_pages'] = $total_num_of_pages;
		$data['current_page'] = ($current_page == 0) ? 1 : $current_page;
		
		if(count($list_all_entry) > 0){
			$offset = (($current_page - 1) * $no_of_rec_per_page); 
		} else {
			$offset = 0;			
		} // End 
		
		$list_register_all_entery = $this->registers_entry->list_register_all_entery($this->session->id,$this->session->pharmacy_surgery_id, $drug_id_select_box, $no_of_rec_per_page, $offset, $keyword);
		
		$data['list_register_all_entery'] = $list_register_all_entery;
		
		//echo json_encode($data);
		$this->stencil->layout('ajax');	
		$this->stencil->paint('registers/register_load_more',$data);
		
	}//end register_load_more()
	...
	public function add_edit_register_entery_process(){
        ...
        if(!$this->input->post() && !$this->input->post('add_update_btn')) redirect(SURL.'dashboard');
			...
			$this->session->set_flashdata('src', $src);
	...
	public function adjust_entry($entry_id){
		
		$register_entry = $this->registers_entry->get_register_entry($this->session->pharmacy_surgery_id, '', $entry_id);
		
		if($register_entry){
			
			//print_this($register_entry); exit;
			$data['register_entry']  = $register_entry;

			$this->stencil->layout('pgd_detail_ajax_template'); //pgd_detail_ajax_template
			$this->stencil->paint('registers/adjust_entry',$data);
			
		}else{
			echo 'Invalid Access';
		}//end if($register_entry)
		
	}//end adjust_entry($entry_id)
	
	public function adjust_register_entry_process(){

		extract($this->input->post());
		
		$adjust_register_entry = $this->registers_entry->adjust_register_entry($this->session->pharmacy_surgery_id, $entry_id, $this->input->post());
		
		$this->session->set_flashdata('p_no', $p_no);
		$this->session->set_flashdata('src', $src);
		
		if($adjust_register_entry){

			//$this->session->set_flashdata('drug_id',$drug_id);
			$this->session->set_flashdata('ok_message', 'Record updated successfully.');
			redirect(SURL.'registers?t=1');
			
		}else{
			$this->session->set_flashdata('err_message', 'Something went wrong, please try again later.');
			redirect(SURL.'registers?t=1');
		}//end if($adjust_register_entry)
		
	}//end adjust_register_entery_process()
