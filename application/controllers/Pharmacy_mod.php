...
class Pharmacy_mod extends CI_Model {
	...
	public function get_all_org_pharmacies($search ='', $organization_id, $filter, $pharmacy_surgery_id=''){
		...
		foreach($all as $key => $each){
			...
			if($each['manager_id'] != 0){
				...
				if(!empty($manager)){
					...
					$all[$key]['manager_usertype'] = $manager['user_type'];
					$all[$key]['manager_usertype_id'] = $manager['user_type_id'];
					$all[$key]['manager_contact_number'] = $manager['contact_number'];
