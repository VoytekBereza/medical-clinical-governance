...
class Pmr extends MY_Organization_Controller {
	...
	public function search_patient(){
		...
		$data['list_search_patient'] = $this->pmr->search_patient($this->input->post(), $this->session->pmr_pharmacy_surgery_id);
	...
	public function add_edit_patient_process(){
		...
		$verify_if_email_already_exist = $this->pmr->verify_if_email_already_exist($this->input->post('email_address_patient'), $this->session->pmr_pharmacy_surgery_id);
		
		if($verify_if_email_already_exist){
			
			$this->session->set_flashdata('err_message', 'Patient already exist in the pharamcy with a given email address.');
			redirect(base_url().'organization/pmr/add-edit-patient');
			
		}//end if($verify_if_email_already_exist)
	...
	public function email_exist_patient(){
		...
		$result = $this->pmr->verify_if_email_already_exist($this->input->post('email'), $this->session->pmr_pharmacy_surgery_id);
		...
		if($this->session->user_type == 1 || $this->session->user_type == 2 || $this->session->user_type == 3 ){
			...
			if(($order_details['order_status'] == 'DS' || $order_details['order_status'] == 'C') && $order_details['order_type'] == 'ONLINE'){
				...
				if($order_details['default_prescriber']){
					...
					//Grab all information from the defalt prescriber 
					//This order is apprived by Default prescriber so we have to use the address of the default prescriber

					$get_prescriber_details = $this->pmr->get_prescriber_details($order_details['prescribed_by']);
					$data['prescriber_details'] = $get_prescriber_details;
					
					$data['p_prescriber_name'] = filter_name($order_details['p_prescriber_name']);
					$data['p_prescriber_reg_no'] = filter_name($order_details['p_prescriber_reg_no']);
					$data['p_prescriber_email'] = filter_name($order_details['p_prescriber_email']);
					
					$data['registration_type'] = filter_string($get_prescriber_details['reg_type']);
					$data['user_signatures'] = filter_string($get_prescriber_details['signature_file']);

					$presc_address = filter_string($get_prescriber_details['organization_name']).', ';
					$presc_address .= filter_string($get_prescriber_details['address_1']);
					$presc_address .= ($get_prescriber_details['address_2']) ? ', '.filter_string($get_prescriber_details['address_2']) : '';
					$presc_address .= ($get_prescriber_details['address_3']) ? ', '.filter_string($get_prescriber_details['address_3']) : '';
					$presc_address .= ($get_prescriber_details['town']) ? ', '.filter_string($get_prescriber_details['town']) : '';
					$presc_address .= ($get_prescriber_details['county']) ? ', '.filter_string($get_prescriber_details['county']) : '';
					$presc_address .= ', '.filter_string($get_prescriber_details['postcode']);
					
					$data['presc_address'] = $presc_address;
					
				
				}else{
					
					//exit('on pmr 713 for NON Default Presriber..... this place is for thse having own prescribers');
					...
					$prescriber_organization = $this->organization->get_organization_details($prescriber_organization[0]['id']);
					
					//print_this($prescriber_organization); exit;
					
					//$data['prescriber_organization'] = $prescriber_organization[0];
					//$data['prescriber_organization'] = $get_prescriber_details['organization_name'];

					$data['p_prescriber_name'] = filter_name($get_user_details['first_name'].' '.$get_user_details['last_name']);
					$data['p_prescriber_reg_no'] = filter_name($get_user_details['registration_no']);
					$data['p_prescriber_email'] = filter_name($get_user_details['email_address']);
					...
					$presc_address = filter_string($prescriber_organization['company_name']).', ';
					$presc_address .= filter_string($prescriber_organization['address']);
					$presc_address .= ', '.filter_string($prescriber_organization['postcode']);
				...
				//echo "Stop on pmr line 707"; exit;
				...
				if($order_details['order_type'] == 'PMR'){
					...
					$data['presc_address'] = filter_string($order_details['p_pharmacy_address']);
					$data['organization_details'] = $this->organization->get_organization_details($order_details['organization_id']);

					
				}else{
					
					exit('764 - pmr - contact admin');
				...
				//echo "Stop on pmr line 718"; exit;
				...
				*/
				...
				$data['p_prescriber_name'] = filter_name($this->session->full_name);
				$data['p_prescriber_reg_no'] = filter_string($this->session->registration_no);
				$data['p_prescriber_email'] = filter_string($this->session->email_address);
				
				$data['registration_type'] = filter_string($this->session->registration_type);
				$data['user_signatures'] = $this->users->get_user_signatures($this->session->id);

				if($is_freetype){
					
					$phrmacy_details = $this->pharmacy->get_pharmacy_surgery_details($this->session->pmr_pharmacy_surgery_id);

					$presc_address = filter_string($phrmacy_details['pharmacy_surgery_name']);
					$presc_address = ($phrmacy_details['address']) ? filter_string($phrmacy_details['address']) : '';
					$presc_address .= ($phrmacy_details['postcode']) ? ', '.filter_string($phrmacy_details['postcode']) : '';

				}else{

					$organization_details = $this->organization->get_organization_details($this->session->pmr_organization_id);
					
					//print_this($organization_details);exit;
	
					$presc_address = filter_string($organization_details['company_name']).', ';
					$presc_address .= filter_string($organization_details['address']);
					$presc_address .= ', '.filter_string($organization_details['postcode']);
			...
			//print_this($data['pharmacy_surgery_details']);			exit;
	...
	public function save_prescription(){
		...
		if($this->input->post('save_approve_request')){
			...
			$order_details = $this->pmr->get_order_details($this->input->post('order_detail_id'));
			
			if(!$order_details['default_prescriber'] && !$order_details['prescribed_by']){
				$saved = $this->pmr->mark_transaction_complete($this->input->post('order_detail_id'), $this->session->id, '0');
	...
	public function download_rx($order_detail_id){
		...
		if($order_details['default_prescriber']){
			...
			$get_prescriber_details = $this->pmr->get_prescriber_details($order_details['prescribed_by']);
			$prescriber_details = $get_prescriber_details;
			
			$p_prescriber_name = filter_name($order_details['p_prescriber_name']);
			$p_prescriber_reg_no = filter_name($order_details['p_prescriber_reg_no']);
			$p_prescriber_email = filter_name($order_details['p_prescriber_email']);
			
			$registration_type = filter_string($get_prescriber_details['reg_type']);
			$user_signatures = filter_string($get_prescriber_details['signature_file']);
			...
			$sig = "<img src='".ASSETS.'prescriber_files/'.filter_string($get_prescriber_details['signature_file'])."' width='200px' height='60px' />";

			$org_logo = "<img src='".ASSETS.'prescriber_files/'.filter_string($get_prescriber_details['stamp_file'])."' width='200px' />";
			...
			$p_prescriber_name = filter_name($get_user_details['first_name'].' '.$get_user_details['last_name']);
			$p_prescriber_reg_no = filter_name($get_user_details['registration_no']);
			$p_prescriber_email = filter_name($get_user_details['email_address']);
			
			$registration_type = filter_string($get_user_details['registration_type']);
			$user_signatures = $this->users->get_user_signatures($get_user_details['id']);
			...
			if($prescriber_organization['org_logo'] != '' && file_exists('./assets/images/org_logo/'.filter_string($prescriber_organization['org_logo'])))
				$org_logo = "<img src='".IMAGES.'org_logo/'.filter_string($prescriber_organization['org_logo'])."' width='200px' />";
		...
		$html = str_replace("[PATIENT_GENDER]", ($order_details['gender'] == 'M')? 'Male': 'Female', $html);
		...
		*/
		...
		$html = str_replace("[REGISTRATION_TYPE]", $registration_type, $html);
		$html = str_replace("[PRESCRIBER_REGISTRATION_NO]", filter_string($p_prescriber_reg_no), $html);
		$html = str_replace("[PRESCRIBER_EMAIL_ADDRESS]", filter_string($p_prescriber_email), $html);
		...
		$html = str_replace("[PRESCRIBER_NAME]",ucwords(filter_string($p_prescriber_name)), $html);
		...
		$html = str_replace("[ORGANIZATION_ADDRESS]", $presc_address, $html);	
		$html = str_replace("[ORG_LOGO]", $org_logo, $html);	
