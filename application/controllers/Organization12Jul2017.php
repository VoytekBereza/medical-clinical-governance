...
class Organization extends MY_Organization_Controller {
    ...
    public function dashboard($page_no = ''){
        ...
        if($this->session->is_owner || $this->user_org_superintendent){
            ...
            if($get_organization_details['superintendent_id']) {
                ...
                if($this->input->get('pharmacy_surgery_search') != ''){
                    
                    //$filter = $this->input->post('pharmacy_surgery_search');
					$filter = $this->input->get('pharmacy_surgery_search');
                    $data['filter_for'] = $filter;

					$total_pharmacies_surgeries = $this->pharmacy->get_all_org_pharmacies($this->input->get('pharmacy_surgery_search'),$organization_id, '', '', '');
                    $pharmacies_surgeries = $this->pharmacy->get_all_org_pharmacies($this->input->get('pharmacy_surgery_search'),$organization_id, '', '', $limit, $page);
                
                } else {
                
                    // List Organization Pharmacies / Surgeries
                    if( $this->input->get('list_pharmacies_surgeries_filter') != ''){
    ...
    */
	...
	public function website_payment_process(){
		...
		$td_verify_pharmacy_already_exist = td_verify_pharmacy_already_exist($this->session->email_address);
		$td_verify_pharmacy_already_exist = json_decode($td_verify_pharmacy_already_exist);
		
		if($td_verify_pharmacy_already_exist && !$is_renewal){
			...
			$cdtCardDetailsTransaction->getCustomerDetails()->getBillingAddress()->setAddress1($Address1); //$Address1 '32 Edward Street'
			$cdtCardDetailsTransaction->getCustomerDetails()->getBillingAddress()->setAddress2($Address2); //$Address2 'Camborne'
			...
			$cdtCardDetailsTransaction->getCustomerDetails()->getBillingAddress()->setCity($City); //$City 'Cornwall'
			$cdtCardDetailsTransaction->getCustomerDetails()->getBillingAddress()->setState($State);
			$cdtCardDetailsTransaction->getCustomerDetails()->getBillingAddress()->setPostCode($PostCode); //$PostCode  'TR148PA'
		...
		}//end if($td_verify_pharmacy_already_exist)
    ...
    public function website_paypal_process(){
		...
		if($td_verify_pharmacy_already_exist){
			...
			$items['p_type'] = 'Pharmacy Website';
			$to_buy['desc'] = 'Voyager Medical Website Purchase';
			
			$to_buy = array(
				'currency' => CURRENCY, 
				'type' => PAYMENT_METHOD, 
				'return_URL' => SURL.'organization/website-paypal-checkout-success/'.$pharmacy_id.'/'.$package_id.'/'.$template_id.'/'.urlencode(base64_encode($domain_name)), 
				// see below have a function for this -- function back()
				// whatever you use, make sure the URL is live and can process
				// the next steps
    ...
    public function website_paypal_checkout_success($pharmacy_id, $package_id, $template_id, $domain_name){
        ...
        if(isset($get_ec_return['ec_status']) && ($get_ec_return['ec_status'] === true)) {
            ...
            if (isset($do_ec_return['ec_status']) && ($do_ec_return['ec_status'] === true)) {
                ...
                if($get_ec_return['ACK'] == 'Success' && $do_ec_return['ACK'] = 'Success'){
					...
					$post_data['domain_name'] = base64_decode(urldecode($domain_name));
	...
	public function website_avicenna(){
		...
		}//end if($td_verify_pharmacy_already_exist && !$is_renewal)
				...
				if($copy_status){
					...
					$already_exists_pharmacy_arr = array();
					 ...
					 for($i=1; $i<=count($pharmacy_data); $i++){
						...
						$pharmacy_already_exist = $this->pharmacy->get_pharmacy_by_name_postcode($this->session->organization['id'], $pharmacy_data[$i]['LocationName'], $pharmacy_data[$i]['LocationPostcode']);
					...
					// print_this($new_pharmacy_arr); exit;
