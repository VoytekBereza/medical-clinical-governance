...
class Nhs_comissioning extends MY_Dashboard_Controller {
	...
	public function index(){
		...
		if(!$this->session->id) redirect(base_url().'dashboard');
		 ...
		 $this->breadcrumbcomponent->add('NHS Comissioning', base_url().'nhs-comissioning');
		...
		//CMS DATA
		$cms_data_arr = $this->cms->get_cms_page('nhs-comissioning');
		...
		// Get all quick forms
		$data['nhs_comissioning_arr'] = $this->nhs_comissioning->get_all_nhs_comissioning();
		...
		$this->stencil->paint('nhs_comissioning/nhs_comissioning', $data);
