...
class Pgds_mod extends CI_Model {
  ...
  public function get_pgd_rechas_list($pgd_id, $rechas_type = 'new'){

    $this->db->dbprefix('pgd_rechas');
    $this->db->where('pgd_id',$pgd_id);
    
    if($rechas_type != '')
      $this->db->where('rechas_type',$rechas_type);
...
FROM `kod_pgd_quizes` WHERE `kod_pgd_quizes`.`pgd_id` = '".$pgd_id."' $prev_condition $skip_condition $next_skip_condition LIMIT 0,1";
  ...
  public function get_pgd_quiz_list($pgd_id){

    $this->db->dbprefix('pgd_quizes');
    $this->db->where('pgd_id',$pgd_id);
    
    $get_result = $this->db->get('pgd_quizes');
    $quiz_arr = $get_result->result_array();
    
    return $quiz_arr;
    
  }//end get_pgd_quiz_list($pgd_id)
  
  //Delete Quiz Results
  public function delete_quiz_results($user_id, $order_id = '', $pgd_id = '', $quiz_id = ''){
    ...
    $delate_data = $this->db->delete('pgd_quiz_results');
    
    return $delate_data;
    
  }//end delete_quiz_results($order_id = '', $user_id = '', $pgd_id = '', $quiz_id = '')
  ...
  public function now_get_pgd_subpgds_certificate($user_id,$pgd_id,$subpgd_id, $is_owner_si_org_id = '', $pharmacy_surgery_id = ''){
  ...
  }//end now_get_pgd_subpgds_certificate($user_id,$pgd_id,$subpgd_id)
  ...
  public function auto_quiz_submission_for_package_pgds($user_id, $pgd_id, $order_id){
    
    $created_date = date('Y-m-d G:i:s');
    $created_by_ip = $this->input->ip_address();

    //Delete Previous Attempts against this order, PGD and Userid
    $delete_quiz_results = $this->delete_quiz_results($user_id, $order_id, $pgd_id);
    $get_pgd_quiz_list = $this->get_pgd_quiz_list($pgd_id);
    
    for($i=0; $i<count($get_pgd_quiz_list); $i++){
      ...
      $ins_data = array(
        ...
        'user_id' => $this->db->escape_str(trim($user_id)),
        'quiz_id' => $this->db->escape_str(trim($get_pgd_quiz_list[$i]['id'])),
        'pgd_id' => $this->db->escape_str(trim($pgd_id)),
        'answer_id' => $this->db->escape_str(trim($get_pgd_quiz_list[$i]['correct_option_id'])),
        'answer_status' => $this->db->escape_str('T'),
      ...
      //Inserting User data into the database. 
      $this->db->dbprefix('pgd_quiz_results');
      $ins_into_db = $this->db->insert('pgd_quiz_results', $ins_data);

    }//end for($i=0; $i<count($get_pgd_quiz_list); $i++)
    
    //print_this($ins_data);  exit;
      

    //Quiz is passed now updating/ marking Purchase as quiz passed
    
    $upd_data['is_quiz_passed'] = '1';
    $upd_data['quiz_pass_date'] = date('Y-m-d G:i:s');
    
    $this->db->where('user_id',$user_id);
    $this->db->where('id',$order_id);
    $this->db->dbprefix('user_order_details');
    $ins_into_db = $this->db->update('user_order_details', $upd_data);
  ...
  }//end auto_quiz_submission_for_package_pgds($user_id, $pgd_id, $order_id)
