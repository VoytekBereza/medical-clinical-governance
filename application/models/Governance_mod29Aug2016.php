...
class Governance_mod extends CI_Model {
	...
	public function get_user_governance_data($hr_contract_id){
		...
		$this->db->dbprefix('governance_read_status');
		$this->db->select('governance_read_status.*,organization.company_name, org_pharmacy_surgery.pharmacy_surgery_name, users.first_name, users.last_name,CONCAT(first_name," ",last_name) as user_full_name, users.user_type AS user_type_id, usertype.user_type');
	...
	function mark_organization_sop_read($user_id, $pharmacy_surgery_id='', $organization_sop_detail, $is_superintendent, $user_role=''){
		...
		$ins_data = array(
			...
			//'user_role' => $this->db->escape_str(trim($user_role)),
			'read_dated' => $this->db->escape_str($created_date),
