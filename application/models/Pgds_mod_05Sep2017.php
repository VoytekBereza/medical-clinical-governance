...
FROM `kod_pgd_quizes` WHERE `kod_pgd_quizes`.`pgd_id` = '".$pgd_id."' $prev_condition $skip_condition $next_skip_condition LIMIT 0,1";
	...
	public function get_pgd_subpgds_certificate($user_id,$pgd_id,$subpgd_id, $is_owner_si_org_id = '', $pharmacy_surgery_id = ''){
		...
		if($pgd_access_allowed && $pgd_access_allowed['is_quiz_passed'] && $pgd_access_allowed['doctor_approval'] && $pgd_access_allowed['pharmacist_approval']){
			...
			$get_pharmacist_signatures = $this->users->get_user_signatures(filter_string($pgd_access_allowed['approvedby_pharmacist_id']));// User Signatures
			...
			$get_pharmacist_details = $this->users->get_user_details(filter_string($pgd_access_allowed['approvedby_pharmacist_id']));
			...
			$get_doctor_signatures = $this->users->get_user_signatures(filter_string($pgd_access_allowed['approvedby_doctor_id'])); //Doc signatures
			...
			$get_doctor_details = $this->users->get_user_details(filter_string($pgd_access_allowed['approvedby_doctor_id']));
			...
			//CQC SELECTION
			if($pgd_access_allowed['approved_by_default'] == 1){
				//If PGD is approved by ADMIN Default Doc/ Pharmacist 
	
				//CQC CQC_BODY Name
				$CQC_BODY = 'CQC_BODY';
				$cqc_body_arr = get_global_settings($CQC_BODY); //Set from the Global Settings
				$cqc_body = filter_string($cqc_body_arr['setting_value']); //CQC_BODY
	
				//CQC CQC_MANAGER Name
				$CQC_MANAGER = 'CQC_MANAGER';
				$cqc_manager_arr = get_global_settings($CQC_MANAGER); //Set from the Global Settings
				$cqc_manager = filter_string($cqc_manager_arr['setting_value']); //CQC_MANAGER
	
				$authorized_stamp_path = AUTHORIZED_STAMP_PATH; // Oraganization Stamp
				
			}elseif($is_owner_si_org_id!= ''){
				
				//Check if the Organization have its own default doctor or pharamcist
				if($get_organization_details['default_pharmacist']!= '' && $get_organization_details['default_doctor']!= ''){

					$cqc_body = filter_string($get_organization_details['cqc_body']); //CQC_BODY
					$cqc_manager = filter_string($get_organization_details['cqc_manager']); //CQC_MANAGER
					
					if(filter_string($get_organization_details['cqc_authorized_stamp']) == '')
						$authorized_stamp_path = ORG_STAMP.'default-stamp.png'; // Oraganization Default Stamp
					else
						$authorized_stamp_path = ORG_STAMP.filter_string($get_organization_details['cqc_authorized_stamp']); // Oraganization Stamp
				...
				}//end if($get_organization_details['default_pharmacist']!= '' && $get_organization_details['default_doctor']!= '')
				
			}elseif($pharmacy_surgery_id!= ''){

				$pharmacy_organization_id = $get_pharmacy_details['organization_id'];
				$get_pharmacy_organization_details = $this->organization->get_organization_details(filter_string($pharmacy_organization_id));

				$cqc_pharmacy_name = filter_string($get_pharmacy_organization_details['cqc_pharmacy_name']); //cqc_pharmacy_name
				$cqc_body = filter_string($get_pharmacy_organization_details['cqc_body']); //CQC_BODY
				$cqc_manager = filter_string($get_pharmacy_organization_details['cqc_manager']); //CQC_MANAGER
				
				if(filter_string($get_pharmacy_organization_details['cqc_authorized_stamp']) == '')
					$authorized_stamp_path = ORG_STAMP.'default-stamp.png'; // Oraganization Default Stamp
				else
					$authorized_stamp_path = ORG_STAMP.filter_string($get_pharmacy_organization_details['cqc_authorized_stamp']); // Oraganization Stamp
			}//end if($pgd_access_allowed['approved_by_default'] == 1)
