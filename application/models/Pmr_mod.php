...
class Pmr_mod extends CI_Model {
	...
	public function get_prescriber_details($presc_id){
	...
	}//end get_prescriber_details($patient_id)
	
	
	// Start Function search_patient(): Return Patients
	public function search_patient($data, $pharmacy_id){
		...
		$this->db->select("CONCAT(first_name,' ',last_name,' - ',postcode) as patient_record, patients.id, patients.first_name, patients.last_name, patients.email_address, patient_pharmacies.pharmacy_surgery_id");
		$this->db->from('patients');
		$this->db->where('patients.admin_verify_status',1);
		$this->db->where('patient_pharmacies.pharmacy_surgery_id',$pharmacy_id);
		
		$this->db->where("CONCAT(first_name,' ',  last_name,' ',DATE_FORMAT(dob,'%d%m%Y')) LIKE '%$search_patient%'");
		
		$this->db->join('patient_pharmacies','patients.id = patient_pharmacies.patient_id','LEFT');
	...
	public function add_edit_patient($data, $pharmacy_surgery_id, $organization_id){
		...
		$taken_patient_cosent = ($taken_patient_cosent) ? $taken_patient_cosent : '0';
		...
		if(!$update_patient_id){
			...
			$ins_data = array(
				...
				'taken_patient_cosent' => $this->db->escape_str($taken_patient_cosent),
	...
	public function verify_if_email_already_exist($email_address, $pharmacy_id){

		$this->db->dbprefix('patients');
		$this->db->select('patients.*');
		$this->db->where('patients.email_address', filter_string($email_address));
		$this->db->where('patient_pharmacies.pharmacy_surgery_id', $pharmacy_id);
		$this->db->join('patient_pharmacies','patients.id = patient_pharmacies.patient_id','LEFT');
	...
	*/
	...
	public function mark_transaction_complete($order_id, $prescribed_by = '', $is_default_prescriber = ''){
		
		$upd_arr = array('order_status' => "DS");

		if($is_default_prescriber == '0' && $prescribed_by != ''){
			
			$prescriber_details = $this->users->get_user_details($prescribed_by);
			$upd_arr['prescribed_by'] = $this->db->escape_str(trim($prescribed_by));
			
			$upd_arr['p_prescriber_name'] = $this->db->escape_str($prescriber_details['first_name'].' '.$prescriber_details['last_name']);
			$upd_arr['p_prescriber_reg_no'] = $this->db->escape_str(trim($prescriber_details['registration_no']));
			
			$upd_arr['p_prescriber_email'] = $this->db->escape_str(trim($prescriber_details['email_address']));
			$upd_arr['p_prescriber_contact_no'] = $this->db->escape_str(trim($prescriber_details['mobile_no']));
			
		}//end if($is_default_prescriber == '0')
		
		//print_this($upd_arr); exit;
	...
	public function save_walkin_pgd($patient_id, $pharmacy_surgery_id, $organization_id, $user_id, $data){
		...
		$medicine_details = $this->medicine->get_medicine_by('', $medicine_id);
		
		$medicine_quantities = $this->medicine->get_medicine_quantities($medicine_id,$qty_exploded[0]);
		$medicine_strength = $this->medicine->get_medicine_strength($medicine_id,$strength_id);
		
		$p_strength_name = $medicine_strength['strength'];

		$p_medicine_name = $medicine_details['brand_name'].' '.$medicine_details['medicine_name'].' '.$medicine_strength['strength'].' '.$medicine_quantities['quantity_txt'];
		$medicine_name = $medicine_details['medicine_name'];
		$medicine_form = $medicine_details['medicine_form'];
		$p_suggested_dose = $suggested_dose;
		
		$pharmacy_details = $this->pharmacy->get_pharmacy_surgery_details($pharmacy_surgery_id);
		$p_pharmacy_name = $pharmacy_details['pharmacy_surgery_name'];
		$p_pharmacy_contact_no = $pharmacy_details['contact_no'];
		...
		$p_organization_address = filter_string($organization_arr['address']);
		$p_organization_address .= ', '.filter_string($organization_arr['postcode']);

		$p_organization_name = $organization_arr['company_name'];
		$p_organization_contact_no = $organization_arr['contact_no'];
		...
		$order_details = array(
			...
			'p_medicine_short_name' => $this->db->escape_str(trim($medicine_name)),
			'p_medicine_form' => $this->db->escape_str(trim($medicine_form)),
			'p_suggested_dose' => $this->db->escape_str(trim($p_suggested_dose)),
			
			'p_pharmacy_name' => $this->db->escape_str(trim($p_pharmacy_name)),
			'p_pharmacy_contact_no' => $this->db->escape_str(trim($p_pharmacy_contact_no)),
			'p_pharmacy_address' => $this->db->escape_str(trim($pharmacy_address)),
			
			'p_organization_name' => $this->db->escape_str(trim($p_organization_name)),
			'p_organization_address' => $this->db->escape_str(trim($p_organization_address)),
			'p_organization_contact_no' => $this->db->escape_str(trim($p_organization_contact_no)),
			...
			'p_strength_name' => $this->db->escape_str(trim($p_strength_name)),		
	...
	public function get_patient_order_details($order_id=''){
		...
		$this->db->dbprefix('patient_order_details');
		$this->db->select('patient_order_details.*, patient_orders.prescription_no');
		
		$this->db->join('patient_orders', 'patient_orders.id = patient_order_details.order_id', 'LEFT');
		$this->db->where('patient_order_details.id', $order_id);
